...
class RiseThenDropAnalyzerTest {
    ...
    private RiseThenDropAnalyzer analyzer;

    @BeforeEach
    void setUp() {
        AppProperties.AnalysisConfig analysisConfig = new AppProperties.AnalysisConfig();
        AppProperties.RiseThenDropConfig config = new AppProperties.RiseThenDropConfig();
        config.setDays(3);
        analysisConfig.setRiseThenDrop(config);
        ...
        analyzer = new RiseThenDropAnalyzer(dataFetchService, appProperties);
    }

    @Test
    void analyze_shouldFindRiseThenDrop() {
    ...
    @Test
    void analyze_shouldNotMatchWhenTodayRise() {
        List<KlineData> klines = List.of(
                createKline(100, 105),
                createKline(105, 110),
                createKline(110, 115),
